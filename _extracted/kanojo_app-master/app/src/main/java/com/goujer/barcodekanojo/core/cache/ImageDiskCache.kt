...
class ImageDiskCache(val mContext: Context) {
	...
	fun trimToSize(maxSize: Long) {
		...
		val topFiles = mStorageDirectory.listFiles()
		if (mStorageDirectory.isDirectory && topFiles != null && topFiles.isNotEmpty()) {
	...
	private fun listAllChildren(parent: File): ArrayList<File> {
		val allChildren = ArrayList<File>()
		for (child in parent.listFiles() ?: return allChildren) {
